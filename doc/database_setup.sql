...
CREATE TABLE IF NOT EXISTS `match_requested` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=31 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='match_requested holds every client app that wants to match to an opponent. It contains the firebase-token, the game meta information and timestamps for possible timeout, if no match is found';
